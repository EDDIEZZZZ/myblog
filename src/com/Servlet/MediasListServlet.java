...
@WebServlet(name = "MediasListServlet")
public class MediasListServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        try {
            Adminuser adminusersql=(Adminuser) request.getSession().getAttribute("adminuser");
            Connection conn = ConnDB.getConn();
            String sql = "select * from medias where me_author=? order by createtime";
            ...
            request.getRequestDispatcher("admin_index_medias.jsp").forward(request,response);
